...
public class RailwayUI extends JFrame implements ActionListener {
	...
	public void paint(Graphics graf) {
		...
		switch (timechoice) {
			case 1:
				...
				break;
			case 3:
				for (int i = 0; i < leftside3.length; i++) // Looping through the leftside at 3 block - gives initial
															// size
				{
					for (int j = 0; j < 4; j++) // Looping through each row fully
					{
				...
				break;
			case 5:
				for (int i = 0; i < leftside5.length; i++) // Looping through the leftside at 5 block - gives initial
															// size
				{
					for (int j = 0; j < 4; j++) // Looping through each row fully
					{
				...
				break;
			case 7:
				for (int i = 0; i < leftside7.length; i++) // Looping through the leftside at 7 block - gives initial
															// size
				{
					for (int j = 0; j < 4; j++) // Looping through each row fully
					{
				...
				break;
			case 9:
				for (int i = 0; i < leftside9.length; i++) // Looping through the leftside at 7 block - gives initial
															// size
				{
					for (int j = 0; j < 4; j++) // Looping through each row fully
					{
			...
			default:
				...
				for (int i = 0; i < leftside1.length; i++) // Looping through the leftside at 1 block - gives initial
															// size
				{
					for (int j = 0; j < 4; j++) // Looping through each row fully
					{
	...
	public void actionPerformed(ActionEvent ev) {
		selectedblock = block.getSelectedItem(); // The values neeeded to book seats and check for availability taken
													// from text boxes
		...
		if (ev.getSource() == submitbut) {
			...
			if (selectedtime == "13:00") // If they are trying to book a ticket a 1 o'clock
			{
				...
				{
					int iftaken = leftside1[convertedseat1][convertedseat2].isTaken(); // Uses the istaken() method to
																						// see if it is taken
					if (iftaken == 1) {
						JOptionPane.showMessageDialog(null, "This seat is taken and cannot be booked"); // Seat is taken
																										// and outputs
																										// an error
																										// message
					} else if (iftaken == 0) {
						...
						tickets[totaltickets] = new Ticket(tickettype, selectseat, selectedblock, selectedtime); // Creates
																													// a
																													// new
																													// ticket
																													// objects
																													...
																													// tickets
																													// array
						int tempprice = tickets[totaltickets].getSeatPrice(); // Gets the seat price using the
																				// getSeatPrice() method
				...
				{
					int iftaken = midside1[convertedseat1][convertedseat2].isTaken(); // Uses the istaken() method to
																						// see if it is taken
					if (iftaken == 1) {
						JOptionPane.showMessageDialog(null, "This seat is taken and cannot be booked"); // Seat is taken
																										// and outputs
				...
				{
					int iftaken = rightside1[convertedseat1][convertedseat2].isTaken(); // Uses the istaken() method to
																						// see if it is taken
					if (iftaken == 1) {
						JOptionPane.showMessageDialog(null, "This seat is taken and cannot be booked"); // Seat is taken
																										// and outputs
		...
		if (ev.getSource() == paybut) // Pay button to generate tickets
		{
			...
			try {
				...
				for (int i = 0; i < totaltickets; i++) // Looping through the number of tickets booked
				{
					...
					output.write(newline);
					output.write("Ticket Destination: Kolkata");
					output.write(newline);
					output.write(
			...
			// JOptionPane.showMessageDialog(null, "These tickets have been printed out to
			// the file 'tickets.txt'");
			JOptionPane.showMessageDialog(null, "Your Ticket has been printed!!!"); // Success message
	...
	public void display(Graphics graf) {
		...
		switch (seattaken) {
			case 0:
				...
				graf.drawString(Integer.toString(seatnumber), x + 20, y + boxheight * 3 / 4); // Painting the seat with
																								// the seat number
																								// printed inside
				break;
			case 1:
...
class Ticket {
	...
	public Ticket(String type, int seatnumber, String block, String time) {
		...
		if (blockticket == "Left Block") {
			...
			if (type == "Handicap") {
				price = 3; // Setting Handicap price
			}
			if (type == "Child") {
